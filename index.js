...
app.post("/slack/events", async (req, res) => {
  console.log("/slack/events が呼ばれた。");
...
app.post("/slack/actions", async (req, res) => {
  console.log("/slack/actions が呼ばれた。");
  ...
  if (actions && actions[0].action_id.match(/do_/)) {
  ...
  // Modal forms submitted --
    ...
    const data = {
      ...
      userName: user.name,
      branch: view.state.values.branches.chose_branch.selected_option.value,
      releaseMessage: view.state.values.release_message.content.value
    };

    console.log(data);

    // 通知
    message.send(process.env.SLACK_BOT_CH, user.name);
